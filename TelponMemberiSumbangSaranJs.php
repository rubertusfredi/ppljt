...
if (isset($_GET["reservation"]) and $_GET["reservation"]<>"")
{
		...
		$sqlKejadian = "SELECT * FROM tb_laporan a left join tb_pegawai c on a.id_user=c.id_user , tb_detail_laporan b where a.id_laporan=b.id_laporan and keperluan='SUMBANG SARAN' and tgl_detail_created between '".$tgl1."' and '".$tgl2."' order by tgl_detail_created desc";
}
else
{
		$sqlKejadian = "SELECT * FROM tb_laporan a left join tb_pegawai c on a.id_user=c.id_user , tb_detail_laporan b where a.id_laporan=b.id_laporan and keperluan='SUMBANG SARAN' order by tgl_detail_created desc";
}
